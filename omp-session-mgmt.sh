...
#                         Migrate session data when a project directory
...
# Example: /mnt/data/repo/ai/foo -> --mnt-data-repo-ai-foo--
omp_bucket_name() {
    local path="${1#/}"       # strip leading /
    path="${path%/}"          # strip trailing /
    printf '%s' "$path" | sed 's|/|-|g' | sed 's|^|--|; s|$|--|'
}
...
Modes:
  ...
  --move OLD NEW
                Migrate all session data for a project that moved from
...
do_move() {
    ...
    # Normalise: resolve symlinks, collapse "/./" and "//", remove trailing "/".
    old_path=$(realpath -m "$old_path")
    new_path=$(realpath -m "$new_path")

    if [[ "$old_path" == "$new_path" ]]; then
        echo "OLD_PATH and NEW_PATH are the same. Nothing to do."
        exit 0
    fi
    ...
    # ── 3. Move session files from old bucket to new bucket ──────────
    if [[ -d "$old_dir" ]]; then
        shopt -s nullglob
        ...
        if [[ ${#sessions_to_move[@]} -gt 0 ]]; then
            for session_jsonl in "${sessions_to_move[@]}"; do
                ...
                info "Moving session ${session_id} to ${new_bucket}"
    ...
    # Build path prefix for session_file rewrites.
    local old_path_prefix="${SESSIONS_DIR}/${old_bucket}"
    local new_path_prefix="${SESSIONS_DIR}/${new_bucket}"
    local e_old_cwd=$(sql_escape "$old_path")
    local e_new_cwd=$(sql_escape "$new_path")
    local e_old_prefix=$(sql_escape "$old_path_prefix")
    local e_new_prefix=$(sql_escape "$new_path_prefix")
...
if [[ "$MODE" == "merge" ]]; then
    ...
    echo "ERROR: Specify either --merge or --move." >&2
    echo ""
    usage
fi